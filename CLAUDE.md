...
AI-powered ticket management system: receives support emails, uses an LLM to classify/summarize/polish replies and **auto-resolve tickets from a knowledge base**, and gives agents a dashboard to manage tickets. See `project-scope.md` (scope), `tech-stack.md` (intended stack), and `implementation-plan.md` (phased build-out). Implemented so far: project scaffolding, authentication (Better Auth email/password, `admin`/`agent` roles), admin user management (CRUD over `/users`), inbound email ingestion, the agent-facing ticket UI (list + detail), and the AI features — on inbound, off the request path via a pg-boss job queue, a single **triage** step both classifies a ticket and tries to **auto-resolve it from the knowledge base** (`server/knowledge-base.md`); plus on-demand thread summaries and AI polish for draft replies (see AI (server), Background jobs, and Tickets for the status lifecycle). The AI runs on **OpenAI `gpt-5-nano` via the Vercel AI SDK** (`server/src/lib/ai.ts`), which differs from the Claude API named in the planning docs — treat this file as the source of truth for what's implemented. Still unbuilt from Phase 6: admin CRUD for the knowledge base, and AI-suggested reply drafts (persisted as `ai_draft`).
...
- Config: `client/components.json` (shadcn init base color `neutral`, CSS variables, `lucide-react` icons) — but the runtime palette is a **custom cobalt-blue theme**, see Theming & dark mode below.
...
- The color tokens (light/dark CSS vars) + Chrome autofill override live in `client/src/index.css`; the palette is cobalt (see Theming & dark mode). Font: Geist Variable via `@fontsource-variable/geist`.
...
A **cobalt-blue** brand palette with faintly cool (blue-undertoned) neutrals, plus a **semantic status color language** and a user-facing **light / dark / system** theme toggle.

- **Tokens** — all color CSS vars are defined in `client/src/index.css` under `:root` (light) and `.dark` (dark), in `oklch`. `--primary`/`--ring` are cobalt (`oklch(0.55 0.19 256)` light, `0.65 0.17 256` dark); neutrals carry a small chroma on hue ~256/264; `--destructive` stays red; `--chart-1` is cobalt so the dashboard bars follow the brand. Changing the brand color = edit these two blocks only.
- **Dark mode** — `client/src/lib/theme.tsx` exports a `ThemeProvider` + `useTheme()` hook: state is `'light' | 'dark' | 'system'`, persisted to `localStorage` (`helpdesk-theme`), resolving `system` live via `matchMedia`. It toggles the `.dark` class + `color-scheme` on `<html>`. `main.tsx` wraps the app in `<ThemeProvider>` (inside the Sentry boundary); an **anti-FOUC** inline script in `client/index.html` applies the saved/system class before the bundle loads (keep it in sync with `theme.tsx`). The toggle control (Sun/Moon/Monitor) lives in `NavBar.tsx`. Tests that render a component using `useTheme` must wrap it in `<ThemeProvider>` (see `NavBar.test.tsx`).
- **Semantic colors (single source of truth)** — `client/src/lib/ticketMeta.ts` holds, keyed by the `core` constants: `*_LABEL` maps, plus `STATUS_BADGE_CLASS`/`STATUS_DOT_CLASS`, `CATEGORY_BADGE_CLASS`/`CATEGORY_DOT_CLASS` (soft tinted pill + solid dot, with `dark:` variants), and `MESSAGE_ACCENT_CLASS` (a per-sender left-border color). These are Tailwind class strings, **not** shadcn Badge variants. `client/src/components/TicketBadge.tsx` (`StatusBadge`/`CategoryBadge`) is the signature pill — a soft badge + leading color dot — used wherever a status/category is *displayed* (list, detail); the shadcn `Badge` `variant` prop is unchanged. To recolor a status/category, edit only the maps in `ticketMeta.ts`.
...
- Required `server/.env`: `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`; `OPENAI_API_KEY` is required for the AI features (classification/summary/polish throw without it — the queue worker retries then gives up, and the two AI endpoints return `502`); optional `AGENT_EMAIL`/`AGENT_PASSWORD`, `CLIENT_ORIGIN`, `INBOUND_EMAIL_SECRET`, `SENTRY_DSN` (Sentry error reporting — empty disables it; see Error monitoring) (all gitignored).
...
**Status lifecycle** — 6 statuses in two ownership phases (`core/src/ticket.ts`). AI-owned (set only by the triage pipeline, **hidden from the default list**): `new` (default on creation) → `processing` (worker running) → either `ai_resolved` (auto-answered from the KB) or `open` (handed to a human). Agent-owned (set via `PATCH`, shown by default): `open` → `resolved` (agent solved it manually) or `closed`; a customer reply reopens an `ai_resolved`/`resolved`/`closed` ticket back to `open`. The subsets `AGENT_TICKET_STATUSES` (`open`/`resolved`/`closed`) and `AI_OWNED_TICKET_STATUSES` (`new`/`processing`/`ai_resolved`) partition the enum and drive both the list-hiding and the update-schema guard. Messages are `TICKET_MESSAGE_TYPE.inbound` (email ingestion), `agent_reply` (an agent's reply from the detail page), or `ai_reply` (the KB-grounded answer the triage worker posts when it auto-resolves); `ai_draft` isn't produced yet.

**Endpoints** (`server/src/routes/tickets.ts`, `requireAuth`):
- `GET /api/tickets` — paginated list (summary fields + message `_count`). Query params validated by `ticketListQuerySchema`: `sort`/`order` (server-side ordering; `messages` sorts on the relation `_count`), optional `status` (only the agent-facing statuses)/`category`/`q` filters (`q` matches subject OR requester email, case-insensitive, AND-ed), `page`/`pageSize`. **AI-owned statuses (`new`/`processing`/`ai_resolved`) are hidden by default** (`status notIn AI_OWNED_TICKET_STATUSES`) unless an explicit `status` filter is set or `includeAiHandled=true` (the "Show AI-handled" toggle). Responds `{ tickets, total, page, pageSize }`.
...
- `PATCH /api/tickets/:id` — update triage fields, validated by `updateTicketSchema`. Any agent may change `status` (limited to `AGENT_TICKET_STATUSES` — setting an AI-owned status like `ai_resolved` is a `400`) and `category`; `assigneeId` (assign/unassign) is admin-only and a non-null id must be an active agent. Responds with the updated ticket in the `GET /:id` shape.
...
**Inbound webhook** — `POST /api/inbound-email` (`server/src/routes/inbound-email.ts`): provider-agnostic JSON validated by `inboundEmailSchema` (`{ from, subject, text, messageId?, inReplyTo?, references? }`). **Deduped** by `messageId` (retries → `200 { deduped: true }`); **thread-matched** on `inReplyTo`/`references` against `TicketMessage.messageId` (append + reopen if resolved/closed), else create (`201 { created: true }`). On a **create**, it `await`s `enqueueTriageTicket(...)` (a fast, durable pg-boss insert) before responding, so triage (classification + KB auto-resolution) runs off the request path (see Background jobs) — the ticket starts `new`/`general`; reply-appends keep their existing category/status (aside from the reopen). Public (mail providers carry no session cookie) — guarded by `verifyWebhookSecret` (`server/src/lib/webhookAuth.ts`), a constant-time compare of the `X-Webhook-Secret` header vs `INBOUND_EMAIL_SECRET` (`401` on mismatch). try/catch → `500 { error }`.

**Client UI**:
- `client/src/pages/TicketsPage.tsx` — list built on TanStack Table: server-side sort via column headers, status/category `FilterSelect`s (status offers only `AGENT_TICKET_STATUSES`), debounced search, a **"Show AI-handled" `Switch`** (`TicketsFilters` — sends `includeAiHandled=true` to reveal the AI-owned statuses hidden by default), and pagination. Each subject links to its detail page.
- `client/src/pages/TicketDetailPage.tsx` — a plain header block (subject, status/category badges, Requester / Assigned To / Opened / Updated; the status select offers `AGENT_TICKET_STATUSES`, showing a current AI-owned status as a disabled option) distinct from the message thread below, rendered as `TicketMessageCard`s, with a `TicketReplyForm` composer at the bottom that POSTs a reply and drops the updated ticket into the detail cache. Between header and thread, `TicketSummary` offers an on-demand AI summary (POST `/summarize`, re-runs each click, nothing cached); `TicketReplyForm` also has an AI **Polish** button (POST `/polish-reply`, swaps the draft with the result).
- `client/src/lib/ticketMeta.ts` — shared label + semantic color maps for status/category/message-type; rendered by `StatusBadge`/`CategoryBadge` (`TicketBadge.tsx`) and the message-card left accent (the single-source-of-truth reference pattern; see Theming & dark mode). `client/src/lib/format.ts` — `formatDate` / `formatDateTime` (shared `Intl` instances).
...
- Three exported functions, all guarding on `OPENAI_API_KEY` (throw a clean error if unset, so callers surface `502` or the queue worker retries then gives up):
  ...
  - `triageTicket(subject, body, knowledgeBase)` → `{ category, resolved, reply }` in **one structured call** — it both classifies the ticket and decides whether it can be fully auto-resolved from the knowledge base (respecting the KB's escalation rules; `resolved: false`, `reply: ""` → hand to a human). Constrained to the exact shape via the SDK's `Output.object({ schema })` (a Zod object; the `category` field is a `z.enum([...TICKET_CATEGORIES])`). Run by the pg-boss **triage** worker, never inline in a request.
- The knowledge base lives at `server/knowledge-base.md` and is loaded (read-once, memoized) by `getKnowledgeBase()` in `server/src/lib/knowledgeBase.ts`; the triage worker passes it into `triageTicket`.
...
- **AI SDK v7 APIs differ from v3/v4** — pull docs via Context7 (`ai` package). E.g. structured output uses `generateText` + `Output.choice` / `Output.object`, **not** the old `generateObject({ output: 'enum' })`; the handler/output shapes changed across majors.
...
- `startQueue()` — called once from `server/src/index.ts` after `app.listen` (non-fatal: if it can't start, the API still serves — but new tickets then stay `new`/hidden until triaged; surface them via the "Show AI-handled" toggle). It runs `boss.start()`, `boss.createQueue('ticket-triage', { retryLimit: 3, retryBackoff: true })`, and registers the worker.
- The worker handler receives a **batch array** of jobs (`Job<T>[]`) — iterate it; per job it drives the AI-owned lifecycle: mark the ticket `processing`, run `triageTicket`, then either persist `{ category, status: ai_resolved }` **and** create an `ai_reply` message (in one `$transaction`) or persist `{ category, status: open }`. A throw fails the job and lets pg-boss retry per the queue's `retryLimit`; the worker's `catch` first best-effort sets the ticket `open` so a failed run never leaves it stuck (and hidden) in `processing`.
- `enqueueTriageTicket({ ticketId, subject, body })` — a fast, durable insert; the inbound webhook awaits it, then responds. So triage runs **out of band**, survives a server restart, and is retried — none of which the previous in-process fire-and-forget could do.
...
Error reporting on both packages, **non-fatal by design**: each side reads its DSN from env and the SDK simply stays disabled (no events) when the DSN is unset — so dev/tests/E2E need no Sentry account. Both DSNs are empty by default.

- **Server** — `@sentry/bun` (a `server` dep; the app runs on Bun, so **not** `@sentry/node`). `server/src/instrument.ts` calls `Sentry.init({ dsn: SENTRY_DSN, ... })` and is imported on the **very first line** of `server/src/index.ts` (`import "./instrument"`) — it must run before any other module. `Sentry.setupExpressErrorHandler(app)` is registered **after all routers, before `app.listen`**, to capture errors thrown from routes. Off-request-path errors don't hit that handler, so they're captured explicitly: the pg-boss `boss.on("error")` handler, the triage worker's per-job `catch` (tagged with `ticketId`), and the `startQueue()` boot failure all call `Sentry.captureException`. `SENTRY_DSN` in `server/.env` (empty = disabled). Error-focused: `tracesSampleRate: 0`, `enableLogs: true`.
- **Client** — `@sentry/react` (a `client` dep). `client/src/main.tsx` calls `Sentry.init({ dsn: import.meta.env.VITE_SENTRY_DSN, ... })` before `createRoot`, and wraps the app in `<Sentry.ErrorBoundary>` (inside `StrictMode`, around `QueryClientProvider`). `VITE_SENTRY_DSN` in `client/.env` (empty = disabled; the DSN is public and ships in the bundle — only `VITE_`-prefixed vars are exposed). `tracesSampleRate: 0`.
- Source-map upload (`@sentry/vite-plugin` + an auth token) is **not** set up — add it later for readable client stack traces in production.
